...
describe('Binary search tree', () => {
  ...
  describe('Sanity checks', () => {
    ...
    it('Can check a condition against every node in a tree', () => {
      ...
      function test (k, v) { if (k % 2 !== 0) { throw new Error('Key is not even') } }

      t.checkAllNodesFullfillCondition(test);

      [l, r, ll, lr, rl, rr].forEach(node => {
        node.key += 1;
        (() => { t.checkAllNodesFullfillCondition(test) }).should.throw()
        node.key -= 1
      })
    ...
    it('Can check that a tree verifies node ordering', () => {
      ...
      l.key = 12;
      (() => { t.checkNodeOrdering() }).should.throw()
      l.key = 5
    ...
    it('Checking if a tree\'s internal pointers (i.e. parents) are correct', () => {
      ...
      (() => { t.checkInternalPointers() }).should.throw()
      l.parent = t;
      (() => { t.checkInternalPointers() }).should.throw()
      r.parent = t;
      (() => { t.checkInternalPointers() }).should.throw()
